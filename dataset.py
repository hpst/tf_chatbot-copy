...
def preprocess_sentence(sentence):
    #lower the sentence case
    sentence = sentence.lower().strip()
    #Put spaces between puntuations
...
def load_conversations(hparams, lines_filename, conversations_filename):
    ...
    questions, answers = [], []
    with open(conversations_filename,errors='ignore') as file:
        lines = file.readlines()
    for line in lines:
        parts = line.replace('\n', '').split(' +++$+++ ')
...
def tokenize_and_filter(hparams, tokenizer, questions, answers):
  ...
  #above code make Questions now of equal lenghts due to padding,
...
def get_dataset(hparams):
  ...
  print("vocab size",hparams.vocab_size)
  ...
  train_questions, test_questions = percentage_split(questions, [0.7, 0.3])
  train_answers, test_answers = percentage_split(answers, [0.7, 0.3])
